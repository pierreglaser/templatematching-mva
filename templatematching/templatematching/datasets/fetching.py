...
DatasetInfo = namedtuple(
    "DatasetInfo", ["name", "urlinfos", "main_file", "source"]
)
...
FACESDB_CONFIG = DatasetInfo(
    name="facesdb",
    urlinfos=(
        UrlInfo(
            url=(
                "https://ftp.uni-erlangen.de/pub/facedb/BioID-FD-Eyepos-V1.2"
                ".zip"
            ),
            filenames=tuple(f"BioID{i:04d}.eye" for i in range(1521)),
            uncompress=True,
            encoding="utf-8",
        ),
        UrlInfo(
            url=(
                "https://ftp.uni-erlangen.de/pub/facedb/BioID-FaceDatabase-"
                "V1.2.zip"
            ),
            filenames=tuple(f"BioID{i:04d}.pgm" for i in range(1521)),
            uncompress=True,
            encoding="utf-8",
        ),
    ),
    main_file="BioID_0001.pgm",  # for consistency, all files are relevant,
    source="https://ftp.uni-erlangen.de/pub/facedb/",
)
    ...
    * handlers
    ...
    if overwrite:
        ...
        if filenames is not None:
            ...
            is_file_missing = any(
                [
                    not _check_if_exists(
                        os.path.join(data_dir, name), remove=False
                    )
...
def fetch_facesdb():
    ...
    return fetch_dataset(FACESDB_CONFIG, show_progress=True)
